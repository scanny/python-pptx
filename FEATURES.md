...
- `SlideShapes.clear(preserve_placeholders=True)` / `Slide.clear_shapes(preserve_placeholders=True)` — Bulk-remove every top-level shape from the tree; placeholders are preserved by default. Per-shape `.delete()` side effects run in the normal way — `Picture.delete()` drops the image rel; `GraphicFrame.delete()` drops the chart / chartex / OLE (payload + icon) / SmartArt / 3D-model rels; `Movie.delete()` drops the video, media, and poster-frame rels — so parts referenced only by the removed shapes are garbage-collected on the next save via the `iter_parts()` reachability walk. `[Added in 2026.05.0]`
...
- `BaseShape.theme_style_refs` — Read/write a shape's four theme-style references (line / fill / effect indices into `a:fmtScheme`, plus the `a:fontRef` collection key) as a `ThemeStyleRefs` named-tuple, mirroring PowerPoint's "Shape Styles" gallery. Assigning `None` clears the `<p:style>` child. Graphic-frame and group-shape parents raise `ValueError`. Re-assignment preserves any existing `<a:schemeClr>` colors carried by each ref. `[Added in 2026.05.0]`
- `BaseShape.theme_style` — Read/write an eight-field `ThemeStyle` named-tuple combining the four ref `idx` values and the four nested `<a:schemeClr val="…"/>` theme-color choices (`"accent1"`..`"accent6"`, `"bg1"`/`"bg2"`, `"tx1"`/`"tx2"`, …). Extends `theme_style_refs` for round-trip fidelity when a shape's theme tint differs from the `accent1` default PowerPoint writes on gallery picks. Assigning `None` clears the `<p:style>` child. `[Added in 2026.05.0]`
...
- `BaseShape.clone_onto(shape_tree, left=None, top=None)` — Deep-copy this shape onto any target shape tree (another slide in the same presentation, or a slide in a different presentation), re-embed every referenced package part (images, charts, OLE, SmartArt, media, 3D-model blobs) on the target's package, and return the correctly-typed proxy. Optional `left` / `top` override the clone's slide-relative position; group shapes preserve their internal child coordinate system. Placeholders raise `NotImplementedError`. Supports every shape kind: auto-shape, picture, graphic-frame (table/chart/OLE/SmartArt/3D), connector, and group. `[Added in 2026.05.0]`
...
- `TextFrame.word_wrap` / `TextFrame.auto_size` / `TextFrame.vertical_anchor` / `TextFrame.rotation` / `TextFrame.upright` — Layout. `rotation` maps `a:bodyPr/@rot` (degrees; `None` or `0` clears the attribute; issue #133). `upright` maps `a:bodyPr/@upright` — read/write bool, `True` keeps text visually upright even when the enclosing shape is rotated (issue #485). Both `[Added in 2026.05.0]`. Together `word_wrap` and `auto_size` drive the three text-overflow modes PowerPoint offers (issue #623): wrap at the shape edge (`word_wrap = True` ⇒ `a:bodyPr/@wrap="square"`), overflow on a single line (`word_wrap = False` ⇒ `a:bodyPr/@wrap="none"`), or clip anything outside the shape (`auto_size = MSO_AUTO_SIZE.NONE` ⇒ `<a:noAutofit/>`).
...
- `TextFrame.clone_from(other_text_frame) -> Self` — Deep-copy another text frame's entire `txBody` subtree (body properties, default list-style, every paragraph's `a:pPr` and runs with their `a:rPr`, inline `mc:AlternateContent` math equations, trailing `a:endParaRPr`) onto this text frame. The destination `txBody` element is preserved so the enclosing shape's wiring stays intact — only its children are swapped. Works across text frames / shapes / slides / presentations. Returns `self` for chaining. CLO-5. `[Added in 2026.05.0]`
- `BaseShape.replace_text_frame_from(other_shape) -> Self` — Shape-scoped wrapper on `TextFrame.clone_from` (above). Raises `ValueError` when either shape lacks a text frame (connector, chart / table graphic frame, group shape). Replaces the previous fidelity-losing idiom `shape.text_frame.text = src.text_frame.text` (which dropped every per-run font / size / colour / bullet / paragraph-level attribute). Returns `self` for chaining. CLO-4. `[Added in 2026.05.0]`
...
- `Chart.clone_from(source_chart)` — In-place read-mutate counterpart to `SlideShapes.add_chart_from`: replaces this chart's `c:chartSpace` content with a deep clone of `source_chart` while preserving the target chart part's partname (and every slide-side reference to it — the enclosing `p:graphicFrame`, relationships pointing into the chart part, callers holding a `Chart` object). Drops existing rels (embedded xlsx, user-shapes, chart images, theme-override, chart-style / chart-colors), deep-copies the source's `c:chartSpace` with every `r:id` / `r:embed` / `r:link` rewritten against freshly-allocated rels on this part, and re-embeds the source's `.xlsx` workbook as an independent copy (so PowerPoint's "Edit Data" dialog stays wired per-chart). Returns `self` for chaining. Supports same-presentation and cross-presentation sources. CLO-8 follow-up. `[Added in 2026.05.3.dev0]`
...
- `Chart.set_title(text)` / `Chart.set_axis_title(axis, text)` — One-call convenience helpers that ensure the corresponding title element is present, rewrite its text, and return `self` for chaining (issue #764). `axis` is one of `"category"`, `"value"`, or `"secondary_value"`. Passing `None` or `""` removes the title (equivalent to setting `has_title = False`). Usage: `chart.set_title("Q4 Revenue").set_axis_title("value", "USD")`. `[Added in 2026.05.1.dev0]`
...
- `Series.source_range` / `Series.category_range` / `Series.name_range` / `Series.values_sheet_reference` — Read the `c:f` formula text on `c:val/c:numRef` (values), `c:cat/c:strRef` or `c:cat/c:numRef` (category labels), and `c:tx/c:strRef` (series name), respectively. Each returns `None` when the underlying cell reference is absent (e.g. an inline-literal series). `source_range` also has a setter that rewrites the formula text in place, creating a `c:f` child under an existing `c:numRef` when one isn't already present; the cached values under `c:numCache` are intentionally left untouched — call `Chart.update_cached_values()` afterward to reconcile. `values_sheet_reference` returns a `SheetReference(sheet_name, a1_range)` namedtuple parsed from the formula, with quoted sheet names unquoted (`'My Sheet'!$A$1` → `sheet_name="My Sheet"`). `[Added in 2026.05.0]`
...
- `DataLabel.manual_layout` / `DataLabel.set_manual_layout(x, y)` / `DataLabel.clear_manual_layout()` — Pin an individual data label to a fractional `(x, y)` position on the chart area (same behaviour as dragging the label in PowerPoint). Coordinates are floats in the 0.0-1.0 `"factor"` coordinate space used by `c:manualLayout/c:x` and `c:y`, relative to the chart area, with `(0.0, 0.0)` at the top-left. `set_manual_layout` writes `c:dLbl/c:layout/c:manualLayout` with `c:xMode` / `c:yMode` both defaulting to `"factor"`; `clear_manual_layout` removes the `c:layout` subtree so the label reverts to the inherited / chart-type-default position. The read-only `manual_layout` returns a `ManualLayout(x, y)` `NamedTuple` or `None` when no per-point manual position is present. Issues #1024 and #1025. `[Added in 2026.05.1.dev0]`
- `DataLabel.text_from_cells` — Per-point "Value From Cells" data-label source. Read/write `str` formula pointing the data label's text at a worksheet cell range (e.g. `"Sheet1!$D$2"`), matching PowerPoint's Format Data Labels > Label Options > **Value From Cells** option. Reads from `c:dLbl/c:tx/c:strRef/c:f` and returns `None` when no cell reference is set. The setter writes the `c:dLbl/c:tx/c:strRef/c:f` subtree (creating the `c:dLbl` and `c:tx` ancestors in schema order if not present) and removes any pre-existing `c:tx/c:rich` custom-text sibling, since `c:tx` allows exactly one of `c:strRef` or `c:rich`. Assigning `None` removes the `c:strRef` and — if no sibling `c:rich` remains — the empty `c:tx`, reverting the label to the series-level `show_*` flag defaults. PowerPoint refreshes the `c:strCache` under `c:strRef` on open, so saving a file with this attribute set to a single-cell range renders the label text sourced from that cell. The series-wide `c:extLst/c15:datalabelsRange` variant is not modelled in this MVP. Issue #953. `[Added in 2026.05.1.dev0]`
...
- `GraphicFrame.has_chartex` / `GraphicFrame.chartex_type` — Chartex graphic-frame discovery. `has_chartex` is `True` for Office 2016+ extended (`cx:`) charts (funnel, treemap, sunburst, waterfall, histogram, box-and-whisker, map). `chartex_type` returns the raw `cx:series/@layoutId` string — one of `"funnel"`, `"treemap"`, `"sunburst"`, `"waterfall"`, `"boxWhisker"`, `"clusteredColumn"` (histogram), `"paretoLine"`, `"regionMap"` — so callers can distinguish the chart kind without parsing the chartex part themselves, or `None` on non-chartex frames. Structured read/write of chartex charts is still deferred; the chartex part round-trips verbatim. Issues #386 (passthrough umbrella) and #1047 (box-and-whisker). `[Added in 2026.05.1.dev0]`
