...
shape, `Movie.blob` / `Movie.ext` / `Movie.content_type` for *reading*
the embedded media bytes and MIME type, `Movie.start_time` /
`Movie.start_condition` for controlling auto-play, `Movie.delete()` that
correctly removes the timing-tree entry and the three media-related
