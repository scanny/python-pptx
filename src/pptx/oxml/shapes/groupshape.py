...
class CT_GroupShape(BaseShapeElement):
    ...
    def add_pic_link(
        ...
        """Append a `p:pic` shape that links (rather than embeds) an image.

        The emitted `a:blip` element carries `r:link="rId..."` referencing an external
        relationship whose target is the linked image URL.
        """
        pic = CT_Picture.new_pic_link(id_, name, desc, rId, x, y, cx, cy)
        self.insert_element_before(pic, "p:extLst")
        return pic
