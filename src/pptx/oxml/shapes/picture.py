...
class CT_Picture(BaseShapeElement):
    ...
    def new_pic_svg(
        ...
        """Return a new `<p:pic>` element wired for an editable SVG.

        The emitted `a:blip` references the rasterized PNG fallback via
        ``r:embed`` and carries an ``a:extLst/a:ext`` child with URI
        ``{96DAC541-7B7A-43D3-8B79-37D633B846F1}`` whose ``asvg:svgBlip``
        points at the editable SVG part via ``r:embed``. This mirrors the
        XML PowerPoint 365 emits when a user inserts an SVG image. See
        issue #358.
        """
        return cast(
            CT_Picture,
            parse_xml(
                cls._pic_svg_tmpl() % (shape_id, name, escape(desc), png_rId, svg_rId, x, y, cx, cy)
            ),
        )
