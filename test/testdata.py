...
class CT_ShapePropertiesBuilder(object):
    """Test data builder for CT_ShapeProperties (spPr) XML element"""
    def __init__(self):
    ...
    def with_prst(self, name='rect'):
        """include prstGeom element with prst="*name*", even if empty"""
        self._prst = name
        return self
    ...
    def prstGeom(self):
        if self.avLst:
            if not self._prst:
                self._prst = 'rect'
            prstGeom = ('  <a:prstGeom prst="%s">\n%s  </a:prstGeom>\n' %
                        (self._prst, self.avLst))
        elif self._prst:
            prstGeom = '  <a:prstGeom prst="%s"/>\n' % self._prst
        else:
            prstGeom = ''
...
def an_spPr():
    """Syntactic sugar to construct a CT_ShapeProperties"""
