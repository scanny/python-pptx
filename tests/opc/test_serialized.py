...
class DescribePackageWriter:
    ...
    def it_provides_a_write_interface_classmethod(
        ...
        _init_.assert_called_once_with(
            ANY, "prs.pptx", relationships_, (part_, part_), None, None
        )
    ...
    def and_it_forwards_zip_date_time_to__init__(
        ...
        _init_.assert_called_once_with(ANY, "prs.pptx", relationships_, (part_,), fixed, None)
    ...
    def it_passes_its_password_through_to_the_constructor(
        ...
        PackageWriter.write("prs.pptx", relationships_, (part_,), None, "s3cret")

        _init_.assert_called_once_with(
            ANY, "prs.pptx", relationships_, (part_,), None, "s3cret"
        )

    def and_it_forwards_both_zip_date_time_and_password_to__init__(
        self, request: FixtureRequest, relationships_: Mock, part_: Mock
    ):
        _init_ = initializer_mock(request, PackageWriter)
        method_mock(request, PackageWriter, "_write")
        ...
        PackageWriter.write("prs.pptx", relationships_, (part_,), fixed, "s3cret")

        _init_.assert_called_once_with(
            ANY, "prs.pptx", relationships_, (part_,), fixed, "s3cret"
        )
