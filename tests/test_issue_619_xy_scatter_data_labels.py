...
def _restore_part_factory():
    ...
    Matches the pattern used in ``tests/test_issue_560_data_label_colors.py`` —
    some tests overwrite ``PartFactory.part_type_for`` entries with Mocks and
    don't restore them, which makes ``prs.slides[0]`` return a Mock.
    """
...
class DescribeIssue619XyScatterDataLabels(object):
    ...
    def it_round_trips_data_labels_through_save_reload(self, _restore_part_factory):
